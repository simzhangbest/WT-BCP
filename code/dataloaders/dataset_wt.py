...
class BaseDataSets(Dataset):
    ...
    def wt_sim(self, image):
        ...
        images = np.stack([low, high], axis=-1)
        return images
    ...
    def __getitem__(self, idx):
        ...
        image = h5f['image'][:]
        label = h5f['label'][:]
        # image label (256, 240)  只需要对image 进行该操作
        # images = self.wt_sim(image)
        sample = {'image': image, 'label': label} # images 相比原来尺寸会减半
        if self.split == "train":
