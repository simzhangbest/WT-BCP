...
from networks.net_factory import net_factory, BCP_xnet_v2
...
def test_single_volume(case, net, test_save_path, FLAGS):
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            input_low, input_high = haar_wavelet_transform(input)
            output_m, output_low, output_high = net(input, input_low, input_high)
            # output = output_low + output_high + output_m # xnetv2 只用了第一个
            out_main= output_m
...
def Inference(FLAGS):
    ...
    snapshot_path = '/mnt/zmy/code/BCP/sim_model/OK/ACDC_xnetv2_7_labeled/20241019022925/self_train_dice8955'
    test_save_path = '/mnt/zmy/code/BCP/sim_model/OK/ACDC_xnetv2_7_labeled/20241019022925/self_train_dice8955/predictions/'
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)
    os.makedirs(test_save_path)
    net = BCP_xnet_v2(1, FLAGS.num_classes)
