...
from networks.XNet import XNet
from networks.XNetv2 import XNetv2
from networks.XNetv2_3D import XNetv2_3D_min, XNetv2_3D
...
def BCP_xnet(in_chns=1, class_num=2, ema=False):
    net = XNet(in_chns, class_num).cuda()
    if ema:
        for param in net.parameters():
            param.detach_()
    return net

def BCP_xnet_v2(in_chns=1, class_num=2, ema=False):
    net = XNetv2(in_chns, class_num).cuda()
    if ema:
        for param in net.parameters():
            param.detach_()
    return net
