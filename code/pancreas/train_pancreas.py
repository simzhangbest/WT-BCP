...
# result_dir = '/mnt/zmy/code/BCP/code/pancreas/result/bcp/' + current_time
result_dir = '/mnt/zmy/code/BCP/code/pancreas/result/bcp/' + '2024-10-22-14-17-37'
mkdir(result_dir)
    ...
    for epoch in tqdm_load(range(1, self_training_epochs+1)):
        ...
        if epoch % st_save_step == 0:
            ...
            if val_dice[0] > max_dice:
                save_net(net, str(save_path / f'dice_{val_dice[0]}_self.pth'))
                save_net(net, str(save_path / f'best_ema_{label_percent}_self.pth'))
                max_dice = val_dice[0]
                max_list = avg_metric

            logger.info('Evaluation: val_dice: %.4f, val_maxdice: %.4f' % (val_dice[0], max_dice))
        ...
        for step, ((img_a, lab_a), (img_b, lab_b), (unimg_a, unlab_a), (unimg_b, unlab_b)) in enumerate(zip(lab_loader_a, lab_loader_b, unlab_loader_a, unlab_loader_b)):
            ...
            loss = loss_1 + loss_2 # 两个 loss 直接相加
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
