...
from dataloaders import haar_wavelet_transform_3d
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                # TODO simzhang
                test_patch_low, test_patch_high = haar_wavelet_transform_3d(test_patch)
                y1, _, _ = net(test_patch, test_patch_low, test_patch_high)
...
def test_all_case(net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None,
    ...
    for image_path in tqdm(image_list):
        ...
        print('id:{}, Dice:{}, Jd:{}, ASD:{}, HD:{}'.format(id, single_metric[0], single_metric[1], single_metric[2], single_metric[3]))
    ...
    return avg_metric, metric_list

def test_all_case_sim(net, test_dataset, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None,
                  preproc_fn=None, DTC=False, nms=0, TMI=0):
    total_metric = 0.0
    metric_list = []
    
    for id in range(len(test_dataset)):
        image = test_dataset[id][0] # [1, 96, 96, 96] ->  [1, 96, 96, 96]
        image = torch.squeeze(image, 0)
        label = test_dataset[id][1] # [96, 96, 96]
        ...
        if save_result:
            ...
            nib.save(nib.Nifti1Image(image[:].astype(np.float32), np.eye(4)), test_save_path + id + "_img.nii.gz")
            nib.save(nib.Nifti1Image(label[:].astype(np.float32), np.eye(4)), test_save_path + id + "_gt.nii.gz")
    avg_metric = total_metric / len(test_dataset)
    return avg_metric, metric_list
...
def test_calculate_metric(net, test_dataset, num_classes=2, dim=(96, 96, 96), s_xy=18, s_z=4, pancreas=True, DTC=False, nms=0):
    ...
    avg_metric, m_list = test_all_case_sim(net, test_dataset, num_classes=num_classes,
