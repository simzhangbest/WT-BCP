...
class XNetv2_3D_min(nn.Module):
    ...
    def forward(self, x_main, x_L, x_H):
        # Main encoder
        M_enc1 = self.M_encoder1(x_main) # x_main [2, 1, 128, 128, 128]
        M_enc2 = self.M_encoder2(self.M_pool1(M_enc1)) # M_enc1 [2, 24, 128, 128, 128]
        M_enc3 = self.M_encoder3(self.M_pool2(M_enc2)) # M_enc2 [2, 48, 64, 64, 64]
        M_enc4 = self.M_encoder4(self.M_pool3(M_enc3)) # M_enc3 [2, 96, 32, 32, 32]

        M_bottleneck = self.M_bottleneck(self.M_pool4(M_enc4)) # M_enc4  [2, 192, 16, 16, 16]  M_bottleneck [2, 384, 8, 8, 8]

        # L encoder
        L_enc1 = self.L_encoder1(x_L) # x_L [2, 1, 128, 128, 128]
        L_enc2 = self.L_encoder2(self.L_pool1(L_enc1)) # L_enc1 [2, 24, 128, 128, 128]
        L_enc3 = self.L_encoder3(self.L_pool2(L_enc2))
        L_enc4 = self.L_encoder4(self.L_pool3(L_enc3))

        L_bottleneck = self.L_bottleneck(self.L_pool4(L_enc4)) # M_enc4  [2, 192, 16, 16, 16]  M_bottleneck [2, 384, 8, 8, 8]

        # H encoder
        H_enc1 = self.H_encoder1(x_H) # x_main [2, 1, 128, 128, 128]
        H_enc2 = self.H_encoder2(self.H_pool1(H_enc1))
        H_enc3 = self.H_encoder3(self.H_pool2(H_enc2))
        H_enc4 = self.H_encoder4(self.H_pool3(H_enc3))

        H_bottleneck = self.H_bottleneck(self.H_pool4(H_enc4)) # H_enc4  [2, 192, 16, 16, 16]  H_bottleneck [2, 384, 8, 8, 8]

        # fusion
        M_H_enc1 = torch.cat((M_enc1, H_enc1), dim=1) # M_H_enc1 [2, 48, 128, 128, 128]
        M_H_enc1 = self.M_H_conv1(M_H_enc1) # M_H_enc1 [2, 24, 128, 128, 128]
        ...
        M_dec1 = self.M_decoder1(M_dec1)
        M_outputs = self.M_conv(M_dec1) # M_outputs [2, 10, 128, 128, 128]
        ...
        return M_outputs, L_outputs, H_outputs  # shape [2, 10, 128, 128, 128]
