...
def var_all_case_LA(testloader, model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
   
    # with open('/mnt/zmy/data/xnet/06_LA/test.list', 'r') as f:
    #     image_list = f.readlines()
    # image_list = ["/mnt/zmy/data/xnet/06_LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]

    total_dice = 0.0
    
    for _, test_batch in enumerate(testloader):
        volumn_batch, label_batch = test_batch['image'], test_batch['label']
        
        volumn_batch = volumn_batch.squeeze(1)

        for index in tqdm(range(volumn_batch.shape[0])):
            image = volumn_batch[index]
            label = label_batch[index]
            label = label.cpu().data.numpy() # 转np
    ...
    #     label = h5f['label'][:]
            ...
            total_dice += dice
        avg_dice = total_dice / volumn_batch.shape[0]
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    y1, _, _ = model(test_patch, test_patch_low, test_patch_high)
